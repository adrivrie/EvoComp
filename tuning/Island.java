...
public class Island implements Cloneable
{
	...
	private ArrayList<Chromosome> recombineDiscrete(Tuple[] matingPool){
		...
		for (Tuple couple : matingPool) {
			...
			boolean doCrossover = rnd_.nextDouble() < model.crossoverRate;
			...
			if (doCrossover) {
				...
				child1.object = parent1.object.clone();
				child2.object = parent2.object.clone();
			}
			...
			if (doCrossover) {
				...
				child1.mutationStepSizes = parent1.mutationStepSizes.clone();
				child2.mutationStepSizes = parent2.mutationStepSizes.clone();
			}
    ...
    private void selectLifetime(ArrayList<Chromosome> offspring) {
        ...
        for(Chromosome individual : population) {
            ...
            //TODO there is a problem when pops become too small/big, this quick fix should be replaced by something proper
            while(newPop.size()<model.initialPopulationSize) {
            	int index = rnd_.nextInt(population.size());
            	newPop.add(population.get(index));
            }
            while(newPop.size()>model.initialPopulationSize*15) {
            	int index = rnd_.nextInt(newPop.size());
            	newPop.remove(index);
            }
        }
